...
| Legacy archaeology (`/legacy-spec`, flow-trace legacy) | Use explicit user path or resolve one entry from `legacy-repos.local.json` |
...
5. Optional accelerator: if ArtifactGraph is installed, use its compact
   project/status tools when available; otherwise stay on the deterministic
   explicit-path / `legacy-repos.local.json` fallback (never invent a missing
   tool name). Follow the Bundlekit rule's run-scoped missing-optional event
   contract after fallback; retries must not produce another event.
