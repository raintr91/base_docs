...
<h3>Toolkit là gì</h3>
<ul>
<li>Một <strong>toolkit</strong> là một package MCP/CLI độc lập: tự cài, tự init, tự chạy. Ví dụ: Docskit, Processkit, Codegenkit, Testkit, ArtifactGraph, CodeGraph, Platform DNA.</li>
...
|---------|---------|---------|
| `/architecture` | arc42 router → chapter / business layer | Docskit |
| `/context` | §03 LND/CTX (overview, actors, operational areas) | Docskit |
| `/containers` | §05 C4 runtime containers (`CTR-*`) + CMP index | Docskit |
| `/component` | `product/components/CMP-*` (module) | Docskit |
| `/journey` | §06 `FLOW-*` (`/dynamics` = deprecated alias) | Docskit |
| `/deployment` | §07 stub-first DEP | Docskit |
...
| FE (Docskit optional) | `DOCSKIT_ROOT` / `--docs-root` | Docskit ID→path |
...
|-----------|-------------|------|
| Author/index arc42 + C4 ID, bundle IR, spec | **docskit** | docs (home); fe/be/tests consumer với `DOCSKIT_ROOT`→docs |
...
### docskit — SSOT for Architecture & Spec

<img class="toolkit-illus" src="./assets/toolkit-docskit.png" alt="Docskit handles architecture indexing and bundle IR" />

```bash
curl -fsSL https://raw.githubusercontent.com/raintr91/docskit/main/install.sh | bash
docskit init      # agents → lane (docs home / consumer) → optional toolkits → MCP + harness
```
...
Accelerator thuần: không toolkit nào bắt buộc nó. Trên repo non-docs nó chỉ index **repo đó** (lexicon baseline đóng gói + `registries/` local), không tự mở docs hub và không đi theo `DOCSKIT_ROOT` / `CODEGENKIT_DOCS_ROOT` / `TESTKIT_DOCS_ROOT`.
...
|------|---------------------|----------------------|---------|
| `docs` | docskit · processkit | **artifactgraph** (home) · codegraph | Registry / architecture hub |
| `fe` | codegenkit · testkit · processkit (impact) | codegraph · docskit (`DOCSKIT_ROOT`→docs) · artifactgraph (hiếm) | Đặt `CODEGENKIT_DOCS_ROOT` |
| `be` | codegenkit · processkit (impact) | codegraph · docskit · artifactgraph (hiếm) | Cùng quy tắc pointer với FE |
...
3. Canonical process skills thuộc **Processkit**: `/business-process-trace` (brownfield evidence) và `/business-impact-review` (blast-radius). `/flow-trace` là redirect deprecated một chu kỳ. `/dynamics` là Docskit→`/journey`, không gộp với process trace. Docskit không sở hữu hai skill này.
4. Docs hub không init codegenkit/testkit mặc định.
5. Docs grill (`/bqa-grill-docs` → `/dev-grill-docs` → `/grill-with-docs`) thuộc Docskit; ArtifactGraph/Codegenkit chỉ là accelerator/handoff, không hard-require.
...
4. Truy cập cross-repo dùng pointer machine-local (`DOCSKIT_ROOT`, `CODEGENKIT_DOCS_ROOT`, …), không suy diễn sibling.
...
Contract chung để build/publish một toolkit MCP (Docskit, Processkit, Codegenkit, Testkit, ArtifactGraph, Platform DNA).
